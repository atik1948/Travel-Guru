...
function BlogPostPage() {
  ...
  usePageMetadata({
    title: post ? `Travel Guru | ${post.title}` : 'Travel Guru | Blog Post',
    description: post?.excerpt || 'Read practical travel ideas and destination-focused stories from Travel Guru.',
  })
    ...
    <section className="py-8 md:py-10">
      <div className="rounded-[32px] bg-[linear-gradient(135deg,#f5efe0_0%,#fbfaf6_34%,#dbe5df_100%)] p-6 md:p-8">
        <article className="mx-auto max-w-[900px] rounded-[26px] bg-white/92 p-7 text-ink shadow-[0_24px_60px_rgba(0,0,0,0.1)] md:p-10">
          ...
          <h1 className="mt-5 max-w-[720px] text-4xl font-semibold leading-[1.08] text-ink md:text-[54px]">
            {post.title}
          </h1>
          <p className="text-muted mt-5 max-w-[760px] text-[15px] leading-8">{post.excerpt}</p>
